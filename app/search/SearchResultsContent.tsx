...
export default function SearchResultsContent({ initialItems, query }: SearchResultsContentProps) {
  ...
  const handleLoadMore = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/search?q=${encodeURIComponent(query)}&page=${page + 1}`);
      if (!response.ok) {
